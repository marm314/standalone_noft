...
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 if(RDMd%range_sep/=0) then
  ...
  if(RDMd%range_sep==1) then
   call dm2_intraJL(RDMd,RDMd%Docc_gamma,sqrt_occ,Dsqrt_occ_gamma,RDMd%DM2_Jsr,RDMd%DM2_Lsr,&
...
!!
!! OUTPUT
!! DM2_J=DM2 elements that use Hartree integrals 
!! DDM2_gamma_J=Derivative of the DM2 elements w.r.t. gamma that use Hartree integrals
!!
!! PARENTS
!!
!! CHILDREN
...
!!
!! FUNCTION
!!  Build from the occ numbers and its derivatives the 2-RDM elements and its derivatives w.r.t. gamma for PNOF5-intra term
...
subroutine dm2_intraJL(RDMd,Docc_gamma,sqrt_occ,Dsqrt_occ_gamma,DM2_J,DM2_L,DDM2_gamma_J,DDM2_gamma_L)
!Arguments ------------------------------------
!scalars
 type(rdm_t),intent(inout)::RDMd
!arrays
...
!-----------------------------------------------------------------------
 do iorb=1,RDMd%NBF_occ
  DM2_J(iorb,iorb)=RDMd%occ(iorb)
  DM2_L(iorb,iorb)=zero
  DDM2_gamma_J(iorb,iorb,:)=Docc_gamma(iorb,:)
  DDM2_gamma_L(iorb,iorb,:)=zero
 enddo
!-----------------------------------------------------------------------
end subroutine dm2_intraJL
!!***
